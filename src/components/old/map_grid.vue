    ...
    drawGrid() {
      ...
      zoomable_layer
        ...
        .on("click", function(d, i) {
          ...
          store.commit("mobilesensor_set_update", [
            "1",
            "2",
            "3",
            "4",
      ...
      d3.csv("data/wangge/Num_wangge_hour.csv", function(error, data_num) {
        d3.csv("data/wangge/Avg_wangge_hour.csv", function(error, data_avg) {
          d3.csv("data/wangge/StdSum_wangge_hour.csv", function(
            error,
            data_std
          ) {
            var pointstemp = [];
            for (var i = 0; i < wangges_location.length; i++) {
              ...
              drawPie(
                [
                  wangges_location[i][0] + 0.5 * xjiange,
                  wangges_location[i][1] + 0.5 * yjiange
                ],
                datatemp_num,
                datatemp_avg,
                datatemp_std,
      ...
      function drawPie(
        ...
        var linear_color_background = d3.scale
          .linear()
          .domain([-1, 40, 70, 1000])
          .range(colorbar1);
        d3.select("#wangge" + wanggeid).style(
          "fill",
          linear_color_background(max)
        );
        var huan = [];
        for (var i = 0; i < 120; i++) {
          huan[i] = 1;
        ...
        zoomable_layer
          ...
          .attr(
            "transform",
            "translate(" +
              (location[0] + margin.left) +
              "," +
              (location[1] + margin.top) +
              ")"
          )
          .selectAll()
          .data(piedata)
