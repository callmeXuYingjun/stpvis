...
function f2(){
d3v3.csv("data/phone/uncertainty/Map_jiance_"+id+"_wangge.csv", function(csvdata) {
  var path_temp;
  var zhongxin;
  var zoom;
  ...
  for(var i=0;i<csvdata.length;i++){
    yichang[i]=csvdata[i].map_junzhi;
  }
...
for(var i=0;i<wangdata.length;i++){
  ...
  function f1(){
    d3v3.csv("data/phone/uncertainty/Map_jiance_"+id+".csv", function(csvdata) {
      d3v3.csv("data/phone/hotmap/hotmap.csv", function(hotdata) {
    var yichang=[];
    var biaozhuncha=[];
    for(var i=0;i<csvdata.length;i++){
        yichang[i]=parseFloat(csvdata[i].map_junzhi);
        biaozhuncha[i]=parseFloat(csvdata[i].map_biaozhuncha);
    }
    var linear_r=d3v3.scale.linear()
    .domain([0,Math.max.apply(null,yichang)])
    .range([2,20]);

  var maxx=Math.max.apply(Math, biaozhuncha);
  var linear = d3v3.scale.linear()
  ...
  function getBoundary(qu,qu_id){ 
    ...
    ply.setFillColor(linear(biaozhuncha[qu_id]));
