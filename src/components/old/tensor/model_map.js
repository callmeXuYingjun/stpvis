...
d3v3.csv("data/phone/model/C"+id+".csv", function(csvdata) {
  ...
  function dmap(index){
    var changgui_C=data_all[models[index][0]];
    var maxx=Math.max.apply(Math, changgui_C[models[index][1]]);
    var minn=Math.min.apply(Math, changgui_C[models[index][1]]);
  var linear = d3v3.scale.linear()
    ...
    var map = new BMap.Map("Down-Horizontal"+(index+2)+"-vertikal2-Horizontal1");
    ...
    function getBoundary(qu,qu_id){ 
      ...
      ply.setFillColor(linear(changgui_C[models[index][1]][qu_id]));
...
function f2(){
  d3v3.csv("data/phone/model/C"+id+"_wangge.csv", function(csvdata) {
  var path_temp;
  var zhongxin;
  var zoom;
  ...
  for(var i=0;i<wangdata.length;i++){
    ...
    ], {strokeColor:"#CCCCCC", strokeWeight:2, strokeOpacity:0.8,fillColor:linear(changgui_C[models[index][1]][i]),fillOpacity:1});  //创建矩形
    rectangle.id =i;
